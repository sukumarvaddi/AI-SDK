...
import { pokemonUISchema } from "@/app/api/structured-array/pokemonSchema";
    ...
    <div className="flex flex-col w-full max-w-2xl pt-12 pb-24 mx-auto">
      ...
      <div className="space-y-8">
        {object?.map((pokemon, index) => (
          <div key={pokemon?.name} className="bg-zinc-50 dark:bg-zinc-800 p-6 rounded-lg shadow-sm">
            <h2 className="text-2xl font-bold mb-4">{pokemon?.name}</h2>
            <p>{index}</p>
            <div className="grid grid-cols-2 gap-4">
              {pokemon?.abilities?.map((ability) => (
                <div key={ability} className="bg-zinc-100 dark:bg-zinc-700 p-3 rounded-md">
                  {ability}
                </div>
              ))}
            </div>
