...
import type { ChatMessage } from "@/app/api/web-search/route";
        ...
        <div key={message.id} className="mb-4">
          ...
          {message.parts.map((part, index) => {
            switch (part.type) {
              ...
              case "tool-webSearchPreview":
                switch (part.state) {
                  // When a tool call is being streamed in and we are receiving input gradually
                  case "input-streaming":
                    return (
                      ...
                      >
                        <div className="text-sm text-zinc-500">🔍 Receiving web search request...</div>
                      </div>
                    );
                  // When the full input to the tool call is available
                  case "input-available":
                    return (
                      <div
                        ...
                        className="bg-zinc-800/50 border border-zinc-700 p-2 rounded mt-1 mb-2"
                      >
                        <div className="text-sm text-zinc-400"> Searching web</div>
                      </div>
                    );
                  // When the tool call output is available (after execution of the tool)
                  case "output-available":
                    return (
                      <div
                        ...
                        className="bg-zinc-800/50 border border-zinc-700 p-2 rounded mt-1 mb-2"
                      >
                        <div className="text-sm text-zinc-400">🔍 Web Search Results</div>
                        <div className="text-sm text-zinc-300">
                          <div>web search result available</div>
                        </div>
                      </div>
                    );

                  // When there was an error during tool execution
                  case "output-error":
                    return (
                      <div
                        key={`${message.id}-getWeather-${index}`}
